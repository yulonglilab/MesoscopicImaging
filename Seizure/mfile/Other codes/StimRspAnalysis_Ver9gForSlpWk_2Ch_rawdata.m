%% StimRspAnalysis_Ver9g
% Running time:20220927
%   ======================================================================================
% 程序修改记录见文末
% Fei Deng,20210517,for response analysis with stimuli
% Fei Deng,20220103, 增加输出resize后的mask图片
% Fei Deng,20220113, 修正“*R_RBCor_Ave.tif”图片实际命名有误的问题
% Fei Deng,20220117, 增加每个session可以有对应的Dark images的处理，从而对应不同的offset图片
% Fei Deng,20220206, 增加沿用其它session的baseline的语句
% Fei Deng,20220208, 修正沿用其它session的baseline的语句时，在多通道情况下的错误
% Fei Deng,20220307, 增加校正方法选择，比值或拟合405后做减法
% Fei Deng,20220312, 增加对Mask图片中单一像素点的邻域填充
% Fei Deng,20220314, 增加SaveFolder文件名的判断，对每个trial修改为IMbasal = mean(IMseriesTrialChS(:,:,1:IdxStim(ti)-IdxStart(ti)-1,Chfi),3);
% Fei Deng,20220314, fit405nm的时候仅使用baseline的数据计算fit函数
% Fei Deng,20220324, 增加对每个像素点的linear unmixing计算
% Fei Deng,2022040？, 增加对REM作为basal的语句
% Fei Deng,20220411, 增加对baseline的校正，如photobleach引起的基线降低
% Fei Deng,20220412, 仔细检查代码，删除冗余代码
% Fei Deng,20220414, 微调baseline校正部分细节，解决parfor循环时占用运存过大报错的问题
% Fei Deng,20220421, Ver8i，优化405fit过程，通过对fit所用的图片先进行高斯滤波降低数据点离散程度，提高fit准确率，并增加对fit效果的抽查
% Fei Deng,20220427,Ver9，增加直接读取校正基线以后保存的*baseCor.datbin文件，并优化了临时baseline图片的生成顺序
% Fei Deng,20220508,Ver9a， 修改Baseline correction部分的parfor相关代码，减小运行内存占用,并增加基于smooth函数校正Baseline的选项
% Fei Deng,20220510,Ver9b， 修改spike2多出数据点的校正代码
% Fei Deng,20220511,Ver9c， 针对以前的数据缺少一些校正数据，增加兼容性，避免报错
% Fei Deng,20220521,Ver9d， 针对仅分析session的一部分图片，分析刺激结果的时候报错进行修改，避免报错
% Fei Deng,20220521,Ver9e， 针对个别像素点接近于0，反应计算结果为Inf导致最终结果受影响的bug，进行置零操作;
%                           F_StatusMeanIM_Ver0改为F_StatusMeanIM_Ver1
% StimRspAnalysis_Ver9f,20220529，修正“4-根据刺激mark确定，session及所有trial的baseline并集”语句相关的报错，此前可能复制粘贴错了，
%                                 引入IdxCorLen变量，为避免光激活时基线校正使用的数据受光激活激发光的影响，所选数据点尽量前移，前移IdxCorLen个数据点
%                                 统一baseline和fit所用的图片帧
% StimRspAnalysis_Ver9g,20220529，由于image J的行列编号从0开始起始，修正相关代码使ROI坐标完全对应
% StimRspAnalysis_Ver9g,20230619，修复了ChNorm = 2重赋值后的bug，通过变量ChNorm0区分开
%   =======================================================================================
%% 手动校正部分数据代码记录
% 5-HT3.0+jR_TS3_20210520_Whisker stim_2.mat中删除了多出来的Stim脉冲数据，
% inverted datad
% Stim.level = 1-Stim.level;
% 即删除了第1-10个数据点
% Stim.times = Stim.times(2:end);
% Stim.level = Stim.level(2:end);
% Stim.length = length(Stim.level);

% fileN = 'E:\DF\Temp Data\20210628_TS3_EM stim\rawdata\Session 18\StimRcd\Green plastic plate_20210628_EM stim_18_1.mat';
% % save(fileN,'EEG','EMG','Imaging','Keyboard','Run','Stim','file','-v7.3');
% save(fileN,'Imaging','Keyboard','Run','Stim','file','-v7.3');
...
%% Parameter setting
MotionCorFlag = 1; % 是否做Motion correction
RegisterFlag = 1; % 是否对不同相机的图片进行配准
BaselineCorFlag = 0; % 是否校正基线，1-使用Matlab toolbox选择函数校正，通常exp2; 2-使用beads函数校正；3-使用smooth函数校正，速度很慢
SpectraUnmixFlag = 1; % 是否做Spectra Unmixing
CorrectMethodFlag = 2; % 结合405 nm激发校正方法，1-比值；2-减法
WholeSessionRspFlag = 1; % 是否分析整个session的response,0-不分析；
StimRspFlag = 0; % 是否根据不同刺激分析response
BasalFlagOfSession = 3;  % 分析整个session所用的baseline图片生成方式：1-指定帧；2-根据刺激mark确定；3.整个session的每个像素点的X%低的数值；4-根据刺激mark确定，session及所有trial的baseline并集；5-根据其它方式标注的baseline帧数确定，如睡眠的REM阶段.  % 20220927改为方式3
IdxBaseList = ones(20,2); % 每个session实际输入并且处理的图片帧数范围，初始置最小值避免报错，第一列为开始帧数，第二列为结束帧数
IdxBaseList(:,2) = 9999999999; % 初始置最大值避免报错
% IdxBaseList(1,:) = floor([1,8800/3]); % 根据实际情况手动设定指定session的baseline对应帧数,按照每个通道的图片编号
forceSaveFlag = 1; % 是否以二进制文件保存IMseries
SaveInSubFolderFlag = 1; % 是否将不同session的结果分别存放在不同文件夹
MFIrecordFlag = 1; % 是否分析整个session的平均荧光强度,0-不分析；
SaveSingleChFlag = [0,0]; % 是否保存每个相机（绿色、红色）的单个通道的反应数据（未校正的反应数据）,0-不保存，1-保存；
...
ImCorRatioFile = 'G:\大视场显微镜\Dual imaging settup\20211017\Plastic plate\Results\CorRatio.mat'; % 20211017至20220613之间的数据使用此校正文件
% ImCorRatioFile = 'G:\大视场显微镜\Dual imaging settup\20220613\Plastic plate\Results\CorRatio.mat'; % 20220613之后的数据使用此校正文件
load(ImCorRatioFile);
...
switch CorrectMethodFlag
    ...
    case 2
        SaveFolder = fullfile(ParentFolder,['ResultsU',rawdataName(8:end),'_fitted405_BaseFlag3']); % 20220927运行
%         SaveFolder = fullfile(ParentFolder,['ResultsU',rawdataName(8:end),'_fitted405_SleepWake']);  % 20220927注释         
end
mkdir(SaveFolder);
